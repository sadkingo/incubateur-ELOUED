                        ...
                        <div class="form-group">
                            ...
                            <select name="project_tracking" id="project_tracking" class="form-control @error('project_tracking') is-invalid @enderror">
                                ...
                                @else
                                    ...
                                    <option value="2">{{ trans('auth/project.project_tracking.create_bmc') }}</option>
                                    <option value="3">{{ trans('auth/project.project_tracking.the_stage_of_preparing_the_prototype') }}</option>
                                    <option value="4">{{ trans('auth/project.project_tracking.write_a_descriptive_model') }}</option>
                                    <option value="5">{{ trans('auth/project.project_tracking.stage_of_registering_a_patent_application') }}</option>
                                    <option value="6">{{ trans('auth/project.project_tracking.obtain_a_certificate_of_registration_for_the_patent_filing_application') }}</option>
                                    <option value="7">{{ trans('auth/project.project_tracking.receiving_reservations_and_amendments_requested_from_INAPI') }}</option>
                                    <option value="8">{{ trans('auth/project.project_tracking.resend_the_amended_form_after_lifting_the_reservations') }}</option>
                                    <option value="9">{{ trans('auth/project.project_tracking.discussion_stage') }}</option>
                                    <option value="10">{{ trans('auth/project.project_tracking.obtained_a_patent_startup') }}</option>
                                @endif
                            </select>
